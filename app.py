...
# ── API 키 로드 ──────────────────────────────────────────
# Streamlit Cloud: st.secrets 우선 사용
# 로컬 개발:       data/.env 파일 fallback
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
load_dotenv(os.path.join(BASE_DIR,  ".env"))

try:
    # Streamlit Cloud 환경: secrets.toml 또는 Cloud Secrets에서 로드
    api_key = st.secrets["OPENAI_API_KEY"]
    IS_CLOUD = st.secrets.get("IS_CLOUD", False)
except Exception:
    # 로컬 환경: secrets.toml 없으므로 .env 파일 사용
...
@st.cache_resource
def process_pdf():
    loader = PyPDFLoader("/Users/hyoeun/Desktop/univ/3'1/인공지능서비스개발1/RAG/2024_KB_부동산_보고서_최종.pdf")
